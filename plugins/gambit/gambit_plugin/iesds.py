...
def run_weak_iesds(game: dict[str, Any], config: dict[str, Any] | None = None) -> dict[str, Any]:
    """Run Weak IESDS (Iterated Elimination of Weakly Dominated Strategies).

    Unlike strict IESDS, this eliminates strategies that are weakly dominated
    (at least as good in all cases, strictly better in at least one).
    ...
    while True:
        rounds += 1
        # Use strict=False for weak dominance
        new_support = gbt.supports.undominated_strategies_solve(support, strict=False)
    ...
    summary = _summarize_weak(eliminated, rounds, surviving)
    return {
        "summary": summary,
        "details": {
            "eliminated": eliminated,
...
def _summarize(
    ...
    return (
        f"{count} strategies eliminated in {rounds_actual} round{'s' if rounds_actual != 1 else ''}"
    )
...
def _summarize_weak(
    ...
    if not eliminated:
        return "No weakly dominated strategies found"
    count = len(eliminated)
    rounds_actual = rounds - 1 if rounds > 1 else rounds
    if count == 1:
        e = eliminated[0]
        return f"Weakly dominated: {e['player']}.{e['strategy']}"
    return f"{count} weakly dominated strategies eliminated in {rounds_actual} round{'s' if rounds_actual != 1 else ''}"
